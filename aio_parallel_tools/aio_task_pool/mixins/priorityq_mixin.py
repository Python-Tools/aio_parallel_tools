...
from aio_parallel_tools.aio_task_pool.exception import NotAvailable, UnknownTaskWeight
...
class PriorityQMixin:
    ...
    async def submit(self, task_func: Callable[[Any], Any], *,
        ...
        Args:
            ...
            weight (int): priority number more small more fast. Defaults to 4.
        ...
        """
        if not (isinstance(weight, int) and 10 > weight > 0):
            raise UnknownTaskWeight("unknown task weight {weight} error, weight must be a int in [1,9]")
        if not self.paused:
            task = self._make_task(task_func, args, kwargs)
            fut = task.fut
            if blocking:
                await self._queue.put((weight, task))
                return await fut
            else:
    ...
    def submit_nowait(self, task_func: Callable[[Any], Any], *,
        ...
        if not self.paused:
            ...
            fut = task.fut
            try:
                self._queue.put_nowait((weight, task))
    ...
    async def close_worker(self) -> None:
        ...
        await self._queue.put((1, WorkerCloseSignal))

    def close_worker_nowait(self) -> None:
        """Send a close signal to a worker with no waiting."""
        self._queue.put_nowait((1, WorkerCloseSignal))
